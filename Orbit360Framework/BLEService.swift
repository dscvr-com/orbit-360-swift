...
public class BLEService: NSObject, CBPeripheralDelegate {
    let p: CBPeripheral
    let onServiceConnected: (CBService) -> Void
    let bleService: CBUUID
    let bleCharacteristic: [CBUUID]
    
    public init(initWithPeripheral peripheral: CBPeripheral, onServiceConnected: @escaping (CBService) -> Void, bleService: CBUUID, bleCharacteristic: [CBUUID]) {
    ...
    public func startDiscoveringServices() {
        if p.services != nil {
