...
public let remoteTopNotificationKey = "remoteTop"
public let remoteBottomNotificationKey = "remoteBottom"
...
public class MotorControl: NSObject, CBPeripheralDelegate {
    ...
    public static let BLEServiceUUID =                CBUUID(string: "69400001-B5A3-F393-E0A9-E50E24DCCA99")
    public static let BLECharacteristicUUID =         CBUUID(string: "69400002-B5A3-F393-E0A9-E50E24DCCA99")
    public static let BLECharacteristicResponseUUID = CBUUID(string: "69400003-B5A3-F393-E0A9-E50E24DCCA99")
    ...
    public func getRotationMatrix() -> GLKMatrix4 {
        ...
        return GLKMatrix4Multiply(GLKMatrix4MakeZRotation(-currentPosition.x), GLKMatrix4MakeXRotation(-currentPosition.y))
    }
    ...
    public func runScript(script: [MotorCommand]) {
    ...
    func stepsToRadiantsX(steps: Int32) -> Float {
        return Float(steps) / Float(MotorControl.motorStepsX) * (.pi * 2)
    }
    
    func radiantsToStepsX(rads: Float) -> Int32 {
        return Int32(rads / (.pi * 2) * Float(MotorControl.motorStepsX))
    }
    
    func stepsToRadiantsY(steps: Int32) -> Float {
        return Float(steps) / Float(MotorControl.motorStepsY) * (.pi * 2)
    }
    
    func radiantsToStepsY(rads: Float) -> Int32 {
        return Int32(rads / (.pi * 2) * Float(MotorControl.motorStepsY))
    }
    ...
    public init(s: CBService, p: CBPeripheral, allowCommandInterrupt: Bool) {
        self.service = s
        self.peripheral = p
    ...
    public func moveXRadiants(rads: Float, speed: Int32) {
        startPosition = currentPosition
    ...
    public func moveX(_ steps: Int32, speed: Int32) {
    ...
    public func moveYRadiants(rads: Float, speed: Int32) {
        startPosition = currentPosition
    ...
    public func moveY(_ steps: Int32, speed: Int32) {
        var command = toByteArray(steps)
        command = command.reversed()
        command.append(UInt8((speed >> 8) & 0xFF))
        command.append(UInt8(speed & 0xFF))
    ...
    public func moveXandYRadiants(radsX: Float, speedX: Int32, radsY: Float, speedY: Int32) {
        startPosition = currentPosition
    ...
    public func moveXandY(_ stepsX: Int32, speedX: Int32, stepsY: Int32, speedY: Int32) {
    ...
    public func peripheral(_ peripheral: CBPeripheral, didUpdateValueFor characteristic: CBCharacteristic, error: Error?) {
        if characteristic.uuid.isEqual(MotorControl.BLECharacteristicResponseUUID) {
            ...
            if (str.uppercased() == MotorControl.topButton) {
                print("top")
                NotificationCenter.default.post(name: Notification.Name(rawValue: remoteTopNotificationKey), object: self, userInfo: nil)
                return
            }
            if (str.uppercased() == MotorControl.bottomButton) {
                print("bottom")
                NotificationCenter.default.post(name: Notification.Name(rawValue: remoteBottomNotificationKey), object: self, userInfo: nil)
                return
            }
        }
    }
